...
/**
 * Build a CSS color string from Figma's normalized 0..1 float channels.
 *
 * Channel values are preserved to full JS-number precision (no rounding).
...
export function rgbaCss(input: { r?: number; g?: number; b?: number; a?: number }): string {
  const r = Math.max(0, Math.min(1, input.r ?? 0)) * 255;
  const g = Math.max(0, Math.min(1, input.g ?? 0)) * 255;
  const b = Math.max(0, Math.min(1, input.b ?? 0)) * 255;
