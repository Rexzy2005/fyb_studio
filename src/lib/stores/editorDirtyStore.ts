 ...
 *
 * The editor sets `dirty = true` on every draft change and clears it back to
 * false only after an explicit admin-initiated save. It also registers a
 * `flushSave` callback the shell can invoke when the admin chooses
 * "Save & continue" from the unsaved-changes prompt.
 *
